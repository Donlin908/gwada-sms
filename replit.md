...
- **Fuite secret webhook Stripe corrigée** : `STRIPE_WEBHOOK_SECRET` était écrit en clair dans `.replit` (`[userenv.shared]`), committé dans l'historique git (retrouvé aussi répliqué dans le remote de backup `gitsafe-backup`). Secret régénéré côté Stripe (endpoint `gwada-sms-webhook` / `we_1TdzbQCi3VTHILCdrLqXQX9w`), retiré de `.replit`, migré en secret Replit chiffré. Utilisateur a régénéré la clé et republié le déploiement VM — nouvelle valeur active en production. Purge de l'historique git non effectuée (décision explicite de l'utilisateur) : la rotation neutralise le risque d'usage de l'ancienne valeur.
- **Faille IDOR critique corrigée — vol de réservation via `/api/user/reservations/claim`** : l'endpoint réassignait `userId` sur n'importe quelle réservation active en connaissant simplement le numéro de téléphone (visible publiquement via `GET /api/numbers`, même pour les numéros réservés par d'autres). Un attaquant pouvait ainsi voler la réservation d'un client payant et lire ses SMS/codes OTP via `/api/messages/:phoneNumberId`. Correctif (`server/routes.ts`) : refus explicite (403) si la réservation appartient déjà à un autre compte, et exigence d'une preuve de possession — le `sessionId` invité (localStorage `gwada_session_id`) utilisé lors du paiement, pas seulement le numéro. Frontend (`client/src/pages/dashboard.tsx`) mis à jour pour transmettre ce `sessionId`. Vulnérabilité et correctif vérifiés en conditions réelles (session forgée signée avec le vrai secret de session, tentative de vol bloquée en 403, réclamation légitime avec le bon `sessionId` acceptée), données de test nettoyées après vérification.
